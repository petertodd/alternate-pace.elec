...
void read_time_from_ds3231(){
  time_secs = from_bcd(read_from_ds3231(ds3231_write_secs)) * secs_to_real_secs_divider;
  time_mins = from_bcd(read_from_ds3231(ds3231_write_mins));
  time_hours = from_bcd(read_from_ds3231(ds3231_write_hours));
...
void write_time_to_ds3231(uint8_t addr){
  ...
  i2c_write(slave_addr_write);
  i2c_write(addr);
  if (addr == ds3231_write_secs)
    i2c_write(to_bcd(time_secs / secs_to_real_secs_divider));
  if (addr == ds3231_write_mins)
    i2c_write(to_bcd(time_mins));
  if (addr == ds3231_write_hours)
    i2c_write(to_bcd(time_hours));
  i2c_stop();
}
